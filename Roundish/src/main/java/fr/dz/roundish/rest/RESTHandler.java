...
public abstract class RESTHandler {
	 ...
	 * @return
	 */
	protected Response handleEntity(Object entity) {
		if ( entity == null ) {
			return Response.status(Response.Status.NO_CONTENT).build();
		} else {
			return Response.status(Response.Status.OK)
				.entity(JsonUtils.convertToJson(entity))
				.build();
		}
	}
	
	/**
	 * Handle an exception and creates the response
	 * TODO Debug the request
	 * @param e
	 * @return
	 */
	protected Response handleException(Throwable e) {
		if ( e instanceof WebApplicationException ) {
			...
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
					.entity(JsonUtils.convertExceptionToJson(e))
					.build();
		}
	}
}
