...
public class ServiceMethodDescriptor {
	...
	private List<ServiceMethodParameterDescriptor> serviceMethodParameterDescriptors;
	 ...
	 * @param method
	 */
	public ServiceMethodDescriptor(ServiceDescriptor<?> serviceDescriptor, Method method) {
	 ...
	 */
	public Object execute(User user, Object... parameters) throws ServiceException {
		try {
			...
			if ( e.getCause() instanceof ServiceException ) {
				...
				throw new WebApplicationException("Error while invoking method "+method.getName()+" from service "+serviceDescriptor.getName(), e);
			}
		} catch(Exception e) {
			throw new WebApplicationException("Error while invoking method "+method.getName()+" from service "+serviceDescriptor.getName(), e);
		}
	 ...
	 * @return
	 */
	private static List<ServiceMethodParameterDescriptor> initParameters(ServiceMethodDescriptor methodDescriptor) {
		...
		for ( int i = 0; i < methodDescriptor.getMethod().getParameterTypes().length; i++  ) {
			...
			if ( parameterAnnotation == null ) {
				throw new WebApplicationException("Parameter "+i+" of method "+methodDescriptor.getMethod().toGenericString()+" has no @"+RESTParameter.class.getSimpleName()+" annotation", Response.Status.INTERNAL_SERVER_ERROR);
			}
			// Store the parameter
			else {
				result.add(new ServiceMethodParameterDescriptor(methodDescriptor, parameterClass, parameterAnnotation.value()));
			}
		}
